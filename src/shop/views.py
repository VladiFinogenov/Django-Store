...
class ReviewCreateView(LoginRequiredMixin, CreateView):
    ...
    def handle_no_permission(self):
        ...
        message = mark_safe(
            f"<p>Необходимо <a href='{login_url}'>авторизоваться</a> для добавления комментариев.</p>"
        )
        messages.info(self.request, message)
        return redirect(reverse('shop:product_detail', kwargs={'pk': self.kwargs.get('pk')}))
